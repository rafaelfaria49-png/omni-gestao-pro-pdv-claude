 ...
 *
 * Adapter XSD por ambiente (`resolveXsdAdapterFromEnv`): sem `FISCAL_XSD_WORKER_URL`, usa o gate de
 * composição e a dependência obrigatória (worker XSD real) está ausente → exit code 2, mesmo com
 * manifesto byte-igual ao golden. Com `FISCAL_XSD_WORKER_URL` na allowlist interna, liga o worker
 * real: `--update-manifest` sob o worker real eleva o golden ao estado completo (passo autorizado
 * H4); sem `--update-manifest`, o manifesto completo diverge do golden parcial ainda vigente → exit
 * 4 (honesto: o golden só é elevado no H4). A consistência do artefato é verificada à parte, pela
 * suíte (`proof.test.ts` · P-10); um golden byte-igual NÃO significa GOAL concluído.
 */
...
async function main(): Promise<void> {
  ...
  const xsdAdapter = resolveXsdAdapterFromEnv()
  const deps = {
