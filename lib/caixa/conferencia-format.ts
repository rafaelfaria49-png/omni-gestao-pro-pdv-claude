...
import { FORMA_CANONICA_LABEL, normalizarFormaPagamento } from "./formas-pagamento-caixa"
...
/** Grafias com rótulo próprio: `multiplo` não é forma, e `boleto` é o nome que o operador escolheu. */
const FORMA_PAGAMENTO_LABEL_EXATO: Record<string, string> = {
  multiplo: "Múltiplas formas",
  boleto: "Boleto",
}

/**
 * Rótulo legível da forma de pagamento. Aceita as grafias reais dos fluxos
 * (`cartao_debito`, `debito`, `"cartão de débito"` → `Débito`); desconhecida volta como veio.
 */
export function formaPagamentoLabel(forma: string | null | undefined): string | null {
  const valor = (forma ?? "").trim()
  if (!valor) return null
  const exato = FORMA_PAGAMENTO_LABEL_EXATO[valor.toLowerCase()]
  if (exato) return exato
  const canonica = normalizarFormaPagamento(valor)
  return canonica ? FORMA_CANONICA_LABEL[canonica] : valor
}
