...
describe("formaPagamentoLabel", () => {
  ...
  it("reconhece as grafias reais dos outros fluxos (O.S., Financeiro, sessao-detalhe)", () => {
    expect(formaPagamentoLabel("debito")).toBe("Débito")
    expect(formaPagamentoLabel("cartão de débito")).toBe("Débito")
    expect(formaPagamentoLabel("Cartão de Crédito")).toBe("Crédito")
    expect(formaPagamentoLabel("credito_vale")).toBe("Crédito / Vale")
    expect(formaPagamentoLabel("vale")).toBe("Crédito / Vale")
    expect(formaPagamentoLabel("a_prazo")).toBe("À prazo")
  ...
  it("devolve null sem forma e o valor original quando desconhecida", () => {
    ...
    expect(formaPagamentoLabel("transferencia")).toBe("transferencia")
  })
})
