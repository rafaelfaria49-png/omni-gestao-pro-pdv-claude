...
export const CLIENT_WRITER_INVENTORY: readonly ClientWriterInventoryEntry[] = [
  ...
  },
  {
    id: "rest-duplicates",
    kind: "DELETE_MERGE",
    path: "app/api/clientes/duplicates/route.ts#GET",
    writes: false,
    notes: "Discovery read-only CAD-R2-018-B (scan/seed, store-scoped, bounded). Não executa merge.",
  },
  {
    id: "rest-merge-plan",
    kind: "DELETE_MERGE",
    path: "app/api/clientes/merge/plan/route.ts#POST",
    writes: false,
    notes: "Preview read-only CAD-R2-018-B via buildMergePlan (plano + fingerprint). Não escreve.",
  },
  {
    id: "rest-merge-execute",
    kind: "DELETE_MERGE",
    path: "app/api/clientes/merge/execute/route.ts#POST",
    writes: true,
    notes: "Adapter fino → ClientMergeService.executeMerge (UMA transaction, gate write/admin, confirmação explícita + fingerprint).",
  },
  {
    id: "hub-duplicate-review",
    kind: "INTERACTIVE",
    path: "app/dashboard/clientes/DuplicateMergeDialog.tsx",
    writes: true,
    notes: "Revisão humana CAD-R2-018-B (lista → lado a lado → sobrevivente explícito → campos → confirmação). Escreve só via rotas de merge.",
  },
  {
    id: "script-reimport",
    kind: "DEAD",
