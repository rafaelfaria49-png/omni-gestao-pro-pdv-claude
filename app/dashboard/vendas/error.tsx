...
export default function DashboardVendasError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[dashboard/vendas] boundary error:", error.message, error.digest)
  }, [error])
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-6 py-24 px-6 text-center">
      ...
      <div className="space-y-1">
        <h2 className="text-lg font-semibold text-foreground">Falha ao carregar o PDV</h2>
        <p className="max-w-sm text-sm text-muted-foreground">
          Ocorreu um erro ao inicializar o ponto de venda. Tente novamente.
        </p>
        {error.digest && (
