...
describe("C. ordem dos guards — nada destrutivo antes (§14/§15)", () => {
  ...
  it("nenhuma mutação acontece antes dos guards", () => {
    // Reconciliação 007C: a reposição direta (`tx.produto.update` +
    // `movimentacaoEstoque.create`) deu lugar ao boundary canônico do CAD-R2-009.
    // O que importa para este contrato não mudou: toda mutação vem DEPOIS dos guards.
...
describe("D. o que NÃO mudou neste corretivo", () => {
  it("efeitos de domínio do estorno seguem intactos", () => {
    for (const efeito of [
      // Estoque agora pelo boundary canônico (CAD-R2-009), com chave de idempotência.
      "applyStockMutationTx(",
      "StockIdempotency.cancelamento",
  ...
  it("007C: a reposição direta de estoque NÃO voltou junto com o merge", () => {
    // Intenção do CAD-R2-009: nunca "setar stock de volta"; sempre movimento
    // compensatório pelo boundary. Reintroduzir o caminho antigo seria regressão.
    expect(rota).not.toContain("tx.produto.update")
    expect(rota).not.toContain("tx.movimentacaoEstoque.create")
  })
