...
| DT-08 | Sem multi-depósito no Estoque | Estoque | P0 | 🔄 | herdada | SPRINT_BL07_FASE1 (Fundação) | Bloqueia adapter Marketplace. ADR-0007 + Fase 0 arquitetura (Gate #1A) ✅. **Fase 1 (Fundação) 🔄 em andamento (02/06)**: autorização concedida; models `Deposito`+`ProdutoDeposito`/migração `0011`/services dormentes/backfill escritos, vitest 14/14; `tsc`/`build` + cutover pendentes. Dívidas deliberadas da Fundação → ver **DT-17** |
| DT-17 | Fundação minimalista: `ProdutoDeposito` pode driftar de `Produto.stock` até a Fase 2 (consumidores PDV/OS não cabeados) + campos diferidos vs ADR-0007 (`custoMedio`/`tipo`/`depositoId` no ledger) | Estoque | P2 | 🆕 | 2026-06-02 | SPRINT_BL07_FASE2 | Atalho consciente da Fase 1 (zero mudança de comportamento). Mitigação: camada **dormente** (nenhum consumidor lê `ProdutoDeposito` em prod) + `db:backfill-deposito` re-runnable re-sincroniza ao `Produto.stock`. Resolve na Fase 2 (cabeia write-paths + `depositoId` NOT NULL) |
