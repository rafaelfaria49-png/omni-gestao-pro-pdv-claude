...
> **Adendo — 19 Set 2026 · PDV · Stock drift residual FINISH (`PDV-STOCK-DRIFT-RESIDUAL-P0-002-FINISH`):** overhang com ≥2 depósitos positivos deixa de cortar um bin à conveniência (`MULTI_DEPOSIT_AMBIGUOUS` / 409). P2002 após writes aborta a transação; replay só em `$transaction` nova. Homolog `visual_dev` V2: C1 abort-before-send e C2 hide-response → `VDA-H02-2026-000001` / `000002`, SKU `P0002FINISH` stock 10→8. Production canônica ainda `buildId=7839c3adc326` (#214), **sem** este FINISH. Identidade da pendência RafaCell: **não confirmada** (URL de Production redigida no pull de env; logs 409 vazios). **Não reenviar** a venda real até merge+promote. `CODE_FIXED` neste branch · `HOMOLOGATION_VERIFIED` no :3012 · `PRODUCTION_VERIFIED=false` · `REAL_SALE_RECOVERED=false`.
