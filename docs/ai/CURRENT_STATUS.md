...
> **Adendo — 19 Set 2026 · Estoque · Entrada bloqueada por drift legado (`ESTOQUE-ENTRADA-DRIFT-RECOVERY-P0-001-ENDTOEND`):** o modal de movimentação falhava em Entrada com `SUM(depósitos)=4 != Produto.stock=0` porque o repair estrutural comprovado existia só na saída PDV (`realinharDepositoAoStock`). Entrada humana agora reconcilia **somente** baseline com autoridade de livro (ou bootstrap de linha-zero) na mesma transação — ledger `estoque-reconcile` separado, depois a mercadoria nova. Overhang SUM>stock **sem livro** continua bloqueado (não escolhe 0 nem 4); o operador confirma o total físico na aba Ajuste, que já fechava o depósito alvo. Sem writer ativo novo (cadastro/OS/import/inventário/PDV/cancelamento/devolução já passam pelo boundary; `lib/os-itens-stock.ts` permanece morto). Sem schema/migration. Sem mutação do SKU real em Production. Recuperação do fone Redmi SKU `3817019322732` fica para o fluxo oficial após deploy.

> **Adendo — 19 Set 2026 · PDV · Lifecycle da tentativa após confirmação (PR #215, continuação FINISH):** a primeira prova C1→C2 **não** valia o fluxo contínuo — C2 reusou a `clientSaleId` de C1 (409) e só avançou após apagar `omnigestao:pdv-sale-attempt:loja-1`. Causa: mapa fingerprint→identidade não era encerrado em `markSaleConfirmed` / Reenviar / merge. Corretivo: `closePersistedSaleAttemptIfIdentity` (loja+identidade, nunca o mapa inteiro), `reconcilePersistedSaleAttempts` (ausência na lista parcial não prova inexistência), `resolvePersistedAttemptForNewSale` recusa identidade já confirmada. Homolog V2 sem limpeza manual: C1 `cs_d663…`/`VDA-H02-2026-000003` (abort→retry auto), C2 `cs_67a9…`/`000004` (hide-response + replay), C3 `cs_5a55…`/`000005`; SKU `P0002FINISH2` stock 10→9→8→7, 3 baixas. Production canônica ainda `buildId=7839c3adc326` (#214), **sem** este FINISH. Pendência RafaCell: **não confirmada**. **Não reenviar** a venda real. `CODE_FIXED` neste branch · `HOMOLOGATION_VERIFIED` no fluxo contínuo C1–C3 · `PRODUCTION_VERIFIED=false` · `REAL_SALE_RECOVERED=false`.

> **Adendo — 19 Set 2026 · PDV · Stock drift residual FINISH (`PDV-STOCK-DRIFT-RESIDUAL-P0-002-FINISH`):** overhang com ≥2 depósitos positivos deixa de cortar um bin à conveniência (`MULTI_DEPOSIT_AMBIGUOUS` / 409). P2002 após writes aborta a transação; replay só em `$transaction` nova. A prova C1/C2 inicial exigiu limpeza manual do mapa de tentativas — **não** comprovava o fluxo contínuo (ver adendo de lifecycle acima). Production canônica ainda `buildId=7839c3adc326` (#214), **sem** este FINISH. Identidade da pendência RafaCell: **não confirmada**. **Não reenviar** a venda real até merge+promote. `PRODUCTION_VERIFIED=false` · `REAL_SALE_RECOVERED=false`.

> **Adendo — 18 Set 2026 · PDV · Stock drift residual P0 (`PDV-STOCK-DRIFT-RESIDUAL-P0-002-ENDTOEND`):** o PR #212 cobria só SUM>stock absorvível no principal. Uma nova pendência no PDV Assistência (R$20 · ~14:17) voltou a `STOCK_INVARIANT_DRIFT`. **Identidade da venda em Production: UNCONFIRMED neste agente** (`DATABASE_URL` redigida; logs 409 não retiveram o corpo). A saída PDV ao vivo agora reconcilia **somente** drift comprovável (livro, linha-zero/bootstrap, overhang absorvível, cache obsoleto) na mesma transação da baixa, com ledger estrutural separado. Ajuste humano fecha no depósito alvo quando o novo saldo cabe (decisão explícita). Casos ambíguos/multi-depósito/principal que não absorve continuam 409 com diagnóstico (produto, saldos, gap, motivo). Writers canônicos de cadastro/OS/import/inventário/cancelamento/devolução já passam pelo boundary — sem writer ativo novo identificado. Sem schema/migration. Sem mutação de saldo em Production neste GOAL. A pendência real só some com Reenviar da mesma `clientSaleId` após o deploy.

> **Adendo — 18 Set 2026 · PDV · Pending sync atomicity P0 (`PDV-PENDING-SYNC-ROOTCAUSE-ATOMICITY-P0-001`):** causa recorrente de `syncPending` em múltiplas lojas/computadores: `STOCK_INVARIANT_DRIFT` (ex. SUM(depósitos)=5 ≠ `Produto.stock`=4) saía como HTTP 500 no writer V2 e o flush de 30s reenviava para sempre. V1/V2 agora devolvem **409 + code**. Saída PDV ao vivo pode realinhar depósito principal **para baixo** quando SUM>stock (cadastros continuam fail-closed). Cliente: PENDING ≠ CONFIRMED — estoque/caixa/ledger/crédito/OS só na confirmação; retry da mesma `clientSaleId` não reaplica; discard 404 recompõe efeitos locais; banner distingue AUTO_RETRY / ACTION_REQUIRED / QUARANTINED. Sem schema/migration. Sem mutação manual de estoque em Production.

> **Adendo — 17 Set 2026 · Fiscal · GOAL 022B documento piloto + classificação 217 (`FISCAL-PILOT-HOMOLOGATION-UNBLOCK-022B`):** sucessor do 022 BLOCKED. Produto/venda/caixa/NotaFiscal/job EMISSAO canônicos em `loja-1` (Neon `omnigestao_prod`). Uma transmissão de documento em HOMOLOGACAO consumiu o ledger one-shot. CONSULTA `NFeConsultaProtocolo4` classificou `NOT_FOUND` (sem retranmissão). `fiscalEnabled=false` · janela git `{null,null,null}` · `SECOND_EMISSION_CREATED=false` · `PRODUCTION_CONTACT=false`. MEI entrou na família Simples do motor F2. Detalhe: [`docs/ai-execution/_evidence/FISCAL-PILOT-HOMOLOGATION-UNBLOCK-022B.md`](../ai-execution/_evidence/FISCAL-PILOT-HOMOLOGATION-UNBLOCK-022B.md).

> **Adendo — 17 Set 2026 · Fiscal · GOAL 022 tentativa 3 bloqueada em fonte de documento (`FISCAL-PILOT-HOMOLOGATION-ACTIVATION-022`):** identidade Neon `omnigestao_prod` conferida (`STORE_MATCH=true` · `CERTIFICATE_MATCH=true` · `CANONICAL_DATABASE_MATCH=true`). `ConfiguracaoFiscalLoja` da `loja-1` reconciliada para `provider=SEFAZ_DIRETO`, `cscId=4`, `cscTokenRef=FISCAL_CSC_TOKEN_LOJA_1`, `ambiente=HOMOLOGACAO`, `modeloFiscal=NFCE`, `fiscalEnabled=false`. Worker XSD do run `34833154637` subiu hardenado em loopback e `createConfiguredXsdWorkerClient` devolveu `VALIDACAO_APROVADA`. A emissão real parou em `PILOT_DOCUMENT_SOURCE_BLOCKED`: zero `NotaFiscal`, zero job `EMISSAO`, zero produto com NCM+CFOP+CSOSN+origem. Janela `PILOT_EMISSION_HOMOLOGATION_WINDOW` permaneceu `{null,null,null}`. `SEFAZ_SOAP_POST_COUNT=0` · `PRODUCTION_CONTACT=false`. Detalhe: [`docs/ai-execution/_evidence/FISCAL-PILOT-HOMOLOGATION-ACTIVATION-022.md`](../ai-execution/_evidence/FISCAL-PILOT-HOMOLOGATION-ACTIVATION-022.md).

> **Adendo — 17 Set 2026 · Cadastros · CAD-R2-008 ClientWriteService:** boundary canônico server-only de escrita de Cliente (`lib/cadastros/client-write-service.ts`). Create/update + auditoria atômicos; dedupe via `client-identity` na mesma transação; revisão humana explícita (sem `force`, sem auto-merge). Writers ativos (Actions, REST POST/PATCH/quick, OS V3 via Action, imports, Smart Genius, inativação em lote) migrados para adapters finos. Hard delete fora. Sem schema/migration. Gap de corrida concorrente permanece até CAD-R2-019 (sem unique por documento). Detalhe: [`docs/ai/CAD_R2_008_CLIENT_WRITE_SERVICE.md`](./CAD_R2_008_CLIENT_WRITE_SERVICE.md).

> **Adendo — 17 Set 2026 · Cadastros · CAD-R2-018-A Client Identity/Dedupe:** fundação canônica de identidade de Cliente em `lib/cadastros/client-identity` (sinais reais `document`/`phone`/`email`/`name`, store como fronteira, documento forte só com DV válido). Política determinística sem auto-merge e sem IA; lookup server-side store-scoped sem vazamento cross-store. Writers de create/update migrados no CAD-R2-008. Sem schema/migration, sem merge destrutivo (018-B), sem unique (019). Detalhe: [`docs/ai/CAD_R2_018_A_CLIENT_IDENTITY.md`](./CAD_R2_018_A_CLIENT_IDENTITY.md).
...
> **Adendo — 06 Set 2026 · Fiscal · GOAL 021 observabilidade fiscal consolidada e bateria ampla de cenários offline (`FISCAL-OBSERVABILITY-SCENARIO-BATTERY-021`):** implementação do serviço consolidado de observabilidade fiscal read-only (`lib/fiscal/observability`), expondo telemetria estruturada das 4 dimensões canônicas (fila, estado incerto/reconciliação, throttling cStat 656 e contingência offline). Superfície interna protegida via endpoint `GET /api/internal/fiscal/observability` com autenticação em tempo constante via `FISCAL_QUEUE_INTERNAL_SECRET` e fail-closed (503 se ausente no env, 401 se inválido, 400 se `storeId` omitido). Bateria de testes offline C01–C10 (`test/fiscal/scenario-battery/fiscal-scenario-battery.test.ts`) 100% verde com 10/10 cenários `FULL_FLOW` executando sobre superfícies e orquestradores canônicos com portas em memória: autorização feliz (100) com persistência real, lote assíncrono (103->105->104->100), duplicidade (204) sem consumo de novo número, indisponibilidade SEFAZ (108/109) fail-closed com bloqueio de retry cego na fila, consulta não constante (217), consumo indevido/throttling (656) com pausa real de fila (`pauseStoreForThrottling`) e zero retry cego, timeout/estado incerto com reconciliação idempotente (`reconcileUncertainDocument`), cancelamento fiscal (110111/135) com validação do prazo canônico legal de 30 minutos (`NFCE_CANCELAMENTO_PRAZO_MS = 30 * 60 * 1000`, corrigindo afirmação errônea prévia de 24h), inutilização de número (102) com lock CAS e prova de não reutilização da numeração pelo alocador, e contingência offline (tpEmis=9) com validação integrada de DANFCE HTML e ESC-POS (80mm/58mm), QR-Code v3 assinado offline (SHA-1/RSA com verificação criptográfica por chave pública) e imutabilidade de payload SHA-256. Contenção estrita de rede P2 auditada (`node:https`, `node:tls`, `node:net`, `node:http`, `globalThis.fetch` fail-closed offline com zero tráfego externo). Auditoria 175 estritamente honrada: política de cStat sem suposições para 203, 208, 215 e 225 (`COVERAGE_GAP_UNMODELED_CSTAT=false`). Prontidão técnica para G-F7 derivada programaticamente sem constantes hardcoded (`READY_FOR_G_F7_REVIEW=true`), com gate mantido estritamente fechado (`G_F7_AUTO_ACTIVATION=false`). `SP_ONLY=true` · `SEFAZ_REQUEST_COUNT=0` · `SEFAZ_SOAP_POST_COUNT=0` · `NFCE_EMISSION_COUNT=0` · `FISCAL_OFF=true` · `PRODUCTION_REQUIRED=false` · `SCHEMA_CHANGED=false` · `MIGRATION_CREATED=false` · `ZERO_NETWORK=true` · `HTTPS_NETWORK_GUARD=true` · `TLS_NETWORK_GUARD=true`. 27/27 testes do GOAL verdes, regressão fiscal 309/309 testes verdes, typecheck zero erros. Detalhe: [`docs/fiscal/FISCAL_021_OBSERVABILITY_BATTERY_REPORT.md`](../fiscal/FISCAL_021_OBSERVABILITY_BATTERY_REPORT.md).**
