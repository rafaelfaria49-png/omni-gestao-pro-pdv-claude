...
> **Adendo — 17 Set 2026 · Cadastros · CAD-R2-018-A Client Identity/Dedupe:** fundação canônica de identidade de Cliente em `lib/cadastros/client-identity` (sinais reais `document`/`phone`/`email`/`name`, store como fronteira, documento forte só com DV válido). Política determinística sem auto-merge e sem IA; lookup server-side store-scoped sem vazamento cross-store. Writers atuais **não** foram migrados (entrada do CAD-R2-008). Sem schema/migration, sem merge destrutivo (018-B), sem unique (019). Detalhe: [`docs/ai/CAD_R2_018_A_CLIENT_IDENTITY.md`](./CAD_R2_018_A_CLIENT_IDENTITY.md).
