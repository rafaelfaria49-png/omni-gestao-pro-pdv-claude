...
O download foi feito diretamente do domínio oficial, validado como ZIP pelo tipo de conteúdo e pela assinatura `PK`, e extraído somente em diretório temporário. No spike nativo posterior autorizado, os cinco XSDs necessários foram copiados byte a byte para `lib/fiscal/xsd-native/schemas/PL_010e_v1.02/NFe/` e conferidos contra este manifesto. Nenhum executável desconhecido foi baixado.
...
Os hashes da tabela usam os bytes brutos oficiais. O ZIP mistura finais de linha: `nfe_v4.00.xsd` e `xmldsig-core-schema_v1.01.xsd` usam CRLF, enquanto os demais usam LF. O Git normaliza arquivos textuais conforme a plataforma. O spike não recodifica os bytes para conferir integridade: aceita somente hashes previamente calculados para as três formas exatas possíveis — original, checkout LF e checkout CRLF.

| Arquivo | Hash LF adicional | Hash CRLF adicional |
|---|---|---|
| `nfe_v4.00.xsd` | `920fd7c04a35b49d0b7f56792e650e63cef76cf1b23f10995b1bbec1f0202774` | igual ao bruto oficial |
| `xmldsig-core-schema_v1.01.xsd` | `78f924e7c9cbeb1e4be900b3b1e7faf2d901972635842980fd43dabb533c512b` | igual ao bruto oficial |
| `leiauteNFe_v4.00.xsd` | igual ao bruto oficial | `75eef473f31f722395c43e89422bde3bf6f0b88392d4d60c2c2bfaa8198ddd94` |
| `tiposBasico_v4.00.xsd` | igual ao bruto oficial | `4698a43d3d607b4457d0416dd672e5d1fd649d5e8f523f87212ecf36156750ba` |
| `DFeTiposBasicos_v1.00.xsd` | igual ao bruto oficial | `2502d4f701eddecb4804267155134a800309573128041c9c8ed126a6bd707dfe` |

Qualquer outro valor falha fechado; os bytes entregues ao parser não são reescritos.
...
- Dependência do spike nativo adicionada: não.
- Runner de produção implementado: não; existe apenas um harness experimental isolado.
