...
| Data | 2026-07-16 |
| Branch (reconciliação original) | `fiscal/goal-005-scope-reconciliation` (commit `d74840c`, **preservada, não sobrescrita**) |
| Branch (reaplicação sobre a main atual) | `fiscal/goal-005-scope-reconciliation-current-main` |
| Worktree (reconciliação original) | `../wt-fiscal-goal-005-reconciliation` (`C:/Projetos/wt-fiscal-goal-005-reconciliation`) |
| `origin/main` na reconciliação original | `ba65cd0c8c7d8f5588282fb6c430beab555bd15e` (base original de `d74840c`) |
| `origin/main` na reaplicação | **avançou** `ba65cd0…` → `4f901968365efc0cded6d9a2e348b9f381c1d0c4` (+6 commits do Contador HUB GOAL 006, read-only) — ver §2 e "Reaplicação de proveniência" |
...
> **Reaplicação de proveniência (2026-07-16 — `FISCAL-GOAL-005-SCOPE-RECONCILIATION-REAPPLY-CURRENT-MAIN`).**
> A reconciliação original (`d74840c`) foi escrita sobre `ba65cd0`. Durante a execução, a `origin/main`
> **avançou** para `4f901968365efc0cded6d9a2e348b9f381c1d0c4` — **seis commits do Contador HUB GOAL 006**
> (readers e superfícies **read-only**; sem escrita/assinatura/emissão Fiscal; sem alterar status Fiscal).
> A auditoria de merge readiness (`audit/fiscal-goal-005-scope-reconciliation-readiness`, commit
> `50a3e0179e1b7165339b91127d2fde70d002e41e`) classificou como **A2**: **interseção zero** entre esses
> seis commits e os sete documentos desta reconciliação; **nenhum** conflito textual; **nenhum** conflito
> semântico; **nenhum** impacto Fiscal. Por **higiene de proveniência**, o conteúdo documental de `d74840c`
> foi **reaplicado** sobre a main atual (sem rebase, cherry-pick ou merge), corrigindo apenas as
> afirmações "`origin/main` não avançou / inalterada". A **definição, os gates e os limites do GOAL-005
> permanecem intactos**; os seis commits do Contador entram **apenas como proveniência** e **não** fazem
> parte do GOAL-005. A branch original `fiscal/goal-005-scope-reconciliation` (`d74840c`) permanece
> **preservada e não sobrescrita**.
...
| Item | Valor |
|---|---|
| `origin/main` no início (reconciliação original `d74840c`) | `ba65cd0c8c7d8f5588282fb6c430beab555bd15e` |
| `origin/main` ao final da reconciliação original | `ba65cd0c8c7d8f5588282fb6c430beab555bd15e` (a main vigente naquela janela) |
| `origin/main` na reaplicação (main atual) | `4f901968365efc0cded6d9a2e348b9f381c1d0c4` — avançou +6 commits do Contador HUB GOAL 006 (read-only); **interseção zero** com os sete documentos; sem conflito textual/semântico; sem impacto Fiscal |
