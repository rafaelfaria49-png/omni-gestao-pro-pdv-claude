...
- **C03 — Duplicidade de chave cStat 204 (`FULL_FLOW`)**: Execução canônica via `transmitWithUncertainStateSafety`: convergência idempotente por chave existente, sem retransmissão cega e sem alocar/consumir um novo número fiscal (`numeroConsumido: true`, `requiresConsultation: true`, `requiresInutilizacao: false`).
