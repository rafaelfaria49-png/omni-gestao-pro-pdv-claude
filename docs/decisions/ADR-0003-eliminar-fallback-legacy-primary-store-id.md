...
**Exceção declarada:** `app/api/financeiro/relatorios/exportar/route.ts` — rota acessada via anchor-tag (`<a href>`), que não envia header. Mantém `|| "loja-1"` com `// TODO F-02-anchor`. Resolver em SPRINT_MULTI_LOJA-S-002 (cookie/proxy de loja).
...
- Exceção `exportar/route.ts` mantém fallback temporário (Sprint_02).
