...
import { useTheme } from "@/components/configuracoes/lovable/contexts/ThemeContext";
...
export function ThemeCard() {
  ...
  return (
    <div className="lovable-settings-theme-grid grid grid-cols-1 gap-7 sm:grid-cols-2 xl:grid-cols-4">
      {THEMES.map((t) => {
        ...
        return (
          <div
            key={t.id}
            className={cn(
              "lovable-settings-theme-tile flex flex-col gap-6 rounded-xl border border-border/70 bg-card/70 p-6 shadow-card transition-smooth hover:border-primary/20 hover:bg-muted/30 hover:shadow-card",
