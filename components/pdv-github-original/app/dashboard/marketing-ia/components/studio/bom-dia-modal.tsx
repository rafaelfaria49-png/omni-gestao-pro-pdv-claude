...
import { STUDIO_MOODS, TEMPLATES } from "./studio-templates";

export type { StudioTemplate } from "./studio-templates";
...
export function BomDiaModal({ open, onOpenChange }: BomDiaModalProps) {
  const { preview, setPreview } = useStudioPreview();
  const { template, activeTake, takeMedia, caption: message, mood, showLogo, showPrice } =
    preview;
  ...
  const setActiveTake = (n: number | ((i: number) => number)) => {
    setPreview((p) => ({
      ...p,
      activeTake: typeof n === "function" ? n(p.activeTake) : n,
    }));
  };

  const setMessage = (v: string) => setPreview((p) => ({ ...p, caption: v }));
  const setMood = (id: (typeof STUDIO_MOODS)[number]["id"]) =>
    setPreview((p) => ({ ...p, mood: id }));
  const setShowLogo = (v: boolean) => setPreview((p) => ({ ...p, showLogo: v }));
  const setShowPrice = (v: boolean) => setPreview((p) => ({ ...p, showPrice: v }));
  ...
  const handleFile = (file?: File | null) => {
    ...
    setPreview((p) => ({
      ...p,
      takeMedia: p.takeMedia.map((m, i) => (i === p.activeTake ? url : m)),
    }));
    ...
    if (activeTake < 2) {
      setTimeout(() => {
        ...
        setPreview((p) => ({
          ...p,
          activeTake: Math.min(2, p.activeTake + 1),
        }));
  ...
  const removeTake = (idx: number) => {
    setPreview((p) => ({
      ...p,
      takeMedia: p.takeMedia.map((m, i) => (i === idx ? null : m)),
    }));
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="p-0 bg-card max-h-[90vh] overflow-y-auto sm:!max-w-[1000px]">
                ...
                >
                  <div
                    className={cn(
                      ...
                      done && !active && "border-primary bg-muted text-primary",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 px-6 pb-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <span className="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-primary text-primary-foreground shadow-lg shadow-[0_0_28px_hsl(var(--color-primary)/0.35)] ring-4 ring-primary/25 transition-transform group-hover:scale-105">
        ...
        <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-3 px-6 py-4 bg-muted/40 border-t border-border">
          ...
          <div className="flex items-stretch">
            <Button
              size="lg"
              className="btn-glow rounded-r-none bg-gradient-primary px-6 text-primary-foreground shadow-glow hover:opacity-95 disabled:opacity-50"
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  size="lg"
                  className="btn-glow rounded-l-none border-l border-primary-foreground/25 bg-gradient-primary px-3 text-primary-foreground shadow-glow hover:opacity-95 disabled:opacity-50"
                  disabled={!allTakesDone}
                  aria-label="Agendar"
                >
