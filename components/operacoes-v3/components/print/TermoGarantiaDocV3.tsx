...
// ============================================================================
// Operações V3 — Fase 1E · Documento dedicado: TERMO DE GARANTIA (A4, cliente).
// Render puro de `TermoGarantiaDocV3`. Cores fixas (documento impresso).
// `id="og-print-root"` é o alvo do CSS de impressão.
// ============================================================================

import type { TermoGarantiaDocV3 as TermoDoc } from "@/lib/operacoes-v3/print-model";
...
export function TermoGarantiaDocV3({ doc }: { doc: TermoDoc }) {
  const { empresa, cliente, equipamento, termo } = doc;
  const equip = [equipamento.marca, equipamento.modelo].filter(Boolean).join(" ");
    ...
    <div id="og-print-root" className="mx-auto w-full max-w-[794px] bg-white px-8 py-6 text-black">
      <header className="flex items-start justify-between gap-4 border-b-2 border-black pb-3">
        ...
        <div className="shrink-0 text-right">
          <p className="text-[12px] font-semibold uppercase tracking-wide text-zinc-500">Termo de Garantia</p>
      ...
      {doc.servicoRealizado.length ? (
        <section className="mt-3">
          ...
          <ul className="ml-4 list-disc text-[12px] text-black">
            {doc.servicoRealizado.map((sv, i) => <li key={i}>{sv}</li>)}
          </ul>
        </section>
      ) : null}

      <section className="mt-3 break-inside-avoid">
        <h2 className="mb-1 border-b border-zinc-400 pb-0.5 text-[11px] font-bold uppercase tracking-wide text-zinc-700">Garantia aplicada</h2>
        <p className="text-[13px] font-semibold text-black">{termo.titulo}{termo.prazoDias ? ` — ${termo.prazoDias} dias a partir da entrega` : ""}</p>
        {termo.semCobertura ? (
          <p className="mt-1 text-[12px] text-black">{termo.observacao}</p>
        ) : (
          <>
            {termo.cobertura.length ? (
              <div className="mt-1">
                <p className="text-[11px] font-medium text-zinc-700">O que está coberto:</p>
                <ul className="ml-4 list-disc text-[12px] text-black">{termo.cobertura.map((c, i) => <li key={i}>{c}</li>)}</ul>
              </div>
            ) : null}
            {termo.exclusoes.length ? (
              <div className="mt-1">
                <p className="text-[11px] font-medium text-zinc-700">O que NÃO está coberto:</p>
                <ul className="ml-4 list-disc text-[12px] text-black">{termo.exclusoes.map((e, i) => <li key={i}>{e}</li>)}</ul>
              </div>
            ) : null}
            {termo.observacao ? <p className="mt-1 text-[11px] text-black">{termo.observacao}</p> : null}
          </>
        )}
      </section>
      ...
      <section className="mt-8 break-inside-avoid">
        <p className="text-[10px] text-zinc-600">
          Declaro estar ciente das condições de garantia descritas neste termo.
        </p>
        <div className="mt-10 grid grid-cols-2 gap-10">
          ...
          <div className="text-center"><div className="border-t border-black pt-1 text-[11px] text-black">Responsável técnico</div></div>
        </div>
