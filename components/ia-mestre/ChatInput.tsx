  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2">
        {SUGGESTIONS.map((s) => (
          <button key={s} onClick={() => onValueChange(s)} className="inline-flex items-center gap-1.5 rounded-full border border-border bg-surface/60 px-3 py-1.5 text-[12px] font-medium text-muted-foreground backdrop-blur-md transition hover:border-primary/40 hover:text-foreground" type="button">
      ...
      <form onSubmit={submit} className="group flex min-h-[64px] items-center gap-3 rounded-2xl border border-border bg-surface/80 p-3 pl-4 shadow-elegant backdrop-blur-md focus-within:border-primary/50 focus-within:shadow-glow">
        ...
        <input value={value} onChange={(e) => onValueChange(e.target.value)} type="text" placeholder="Peça um relatório, uma imagem ou uma campanha…" className="flex-1 bg-transparent px-1 py-3 text-[14px] outline-none placeholder:text-[14px] placeholder:text-muted-foreground" />
        ...
        <button type="submit" disabled={!value.trim() || disabled} className="flex h-12 items-center gap-1.5 rounded-xl bg-gradient-primary px-5 text-[13px] font-semibold text-primary-foreground shadow-elegant transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40">
